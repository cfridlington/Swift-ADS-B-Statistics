...
- `--history-number #` is used to specify a different number of history files generated by `dump1090-mutability`. Swift ADS-B Statistics will handle the 0 indexing of files, so a value of 100 will correctly index history_0.json through history_99.json.
...
This is a dictionary, containing a single key called paths. This is itself a dictionary where each key is an aircraft’s 24-bit ICAO identifiers, formatted as 6 hex digits, and the value is an object with the following fields:
- `adsb` aircraft’s 24-bit ADS-B ICAO identifier.
- `messages` a dictionary that of flight times, formatted in unix epoch, to a list of messages containing positional reporting. *Note: Messages are grouped into flights by checking to see if their transmission times are less than 60 minutes apart.* The message object have the following fields:
...
## Example Data
Examples of the files generated by Swift ADS-B Statistics can be found in the [murdock-flight-tracking](https://github.com/cfridlington/murdock-flight-tracking-data) repository. This data comes from [Christopher Fridlington](https://cfridlington.com)’s ADS-B receiver located in the Lakes Region of New Hampshire.
